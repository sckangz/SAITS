...
def process_each_training_step(results, optimizer, val_dataloader, training_controller, summary_writer, logger):
    ...
    # print('train_step:'+str(state_dict['train_step']))
...
def validate(model, val_iter, summary_writer, training_controller, logger):
    ...
    if (state_dict['save_model'] and args.model_saving_strategy) or args.model_saving_strategy == 'all':
        saving_path = os.path.join(
            args.model_saving, 'model_trainStep_{}_valStep_{}_imputationMAE_{:.4f}.pkl'.
...
def impute_all_missing_data(model, train_data, val_data, test_data):
    ...
    with h5py.File(imputation_saving_path, 'w') as hf:
        hf.create_dataset('imputed_test_set', data=imputed_data_dict['test'].reshape(84000, 3)) # (timestep of test set, dim)
    logger.info(f'Done saving all imputed data into {imputation_saving_path}.')


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--config_path', type=str, help='path of config file',default='./configs/Jupiter_Transformer_base.ini')
    parser.add_argument('--test_mode', dest='test_mode', action='store_true', help='test mode to test saved model',default=True)
    ...
    # 读取原始数据文件以便画图，参数需要与gene_XXX_dataset.py中保持一致，不需要则注释掉
    parser.add_argument("--file_path", help='path of dataset file', type=str,
                        default='dataset_generating_scripts/RawData/Geomagnetic/jupiter')
    ...
    unique_months = df['date_time'].dt.to_period('D').unique()
    selected_as_test = unique_months[1:2]  # select last 1 days as test set
    selected_as_val = unique_months[:1]
    selected_as_train = unique_months[:1]
    test_set = df[df['date_time'].dt.to_period('D').isin(selected_as_test)]
    val_set = df[df['date_time'].dt.to_period('D').isin(selected_as_val)]
    train_set = df[df['date_time'].dt.to_period('D').isin(selected_as_train)]
    scaler = StandardScaler()
    ...
    # assert args.device in ['cpu', 'cuda', 'cuda:0', 'cuda:1'], 'device should be cpu or cuda'
    assert args.device in ['cpu', 'cuda', 'cuda:0'], 'device should be cpu or cuda'
    ...
    if args.test_mode:
        ...
        # if iii == 1:  # 从模型文件加载模型继续训练，不需要则注释掉
        #     # args.model_path = cfg.get('test', 'model_path')
        #     args.model_path = 'checkpoints/Geomagnetic_Transformer_base/models/2023-05-11_T10.04.25/model_trainStep_3735_valStep_415_imputationMAE_0.0312.pkl.pkl'
        #     model = load_model(model, args.model_path, logger)
